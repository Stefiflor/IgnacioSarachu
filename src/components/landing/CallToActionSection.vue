<script setup lang="ts">
// WhatsApp info from user request: CTA fuerte de WhatsApp
const whatsappNumber = "549XXXXXXXXX"; // Reemplazar despues 
const whatsappMessage = "Hola Ignacio, me gustaría reservar una consulta privada para analizar el estado de mi empresa.";
const whatsappLink = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage)}`;
</script>

<template>
  <section id="contacto" class="cta-section section-padding bg-navy-gradient">
    <div class="container cta-container">
      <h2 class="cta-title">
        Tome el control de su futuro financiero, <br/>
        <i class="title-italic">hoy.</i>
      </h2>
      <p class="cta-subtitle">
        Agende una sesión de diagnóstico inicial confidencial. Evaluaremos su caso particular y diagramaremos el plan de acción óptimo para su patrimonio.
      </p>
      
      <div class="cta-actions">
        <a :href="whatsappLink" target="_blank" rel="noopener noreferrer" class="btn-whatsapp">
          <svg class="whatsapp-icon" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 00-.57-.01c-.198 0-.52.074-.792.347-.272.271-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
          </svg>
          Contactar por WhatsApp
        </a>
      </div>
    </div>
  </section>
</template>

<style scoped>
.cta-section {
  text-align: center;
  position: relative;
  overflow: hidden;
  padding: 8rem 0;
}

.cta-container {
  max-width: 800px;
  position: relative;
  z-index: 2;
}

.cta-title {
  color: #FFFFFF;
  font-size: 2.5rem;
  margin-bottom: 1.5rem;
}

@media (min-width: 768px) {
  .cta-title {
    font-size: 3rem;
  }
}

.title-italic {
  font-family: 'Playfair Display', serif;
  font-style: italic;
  font-weight: 600;
  color: var(--color-brand-accent);
}

.cta-subtitle {
  color: rgba(255, 255, 255, 0.85);
  font-size: 1.15rem;
  margin-bottom: 3.5rem;
  line-height: 1.8;
  max-width: 600px;
  margin-left: auto;
  margin-right: auto;
}

.btn-whatsapp {
  display: inline-flex;
  align-items: center;
  gap: 0.75rem;
  background-color: var(--color-brand-accent);
  color: var(--color-brand-primary);
  padding: 1.125rem 2.5rem;
  font-weight: 600;
  font-size: 1.1rem;
  border-radius: var(--radius-sm);
  transition: all var(--transition-normal);
  letter-spacing: 0.02em;
}

.btn-whatsapp:hover {
  background-color: white;
  transform: translateY(-2px);
  box-shadow: 0 10px 25px rgba(0,0,0,0.2);
}

.whatsapp-icon {
  width: 24px;
  height: 24px;
}
</style>
